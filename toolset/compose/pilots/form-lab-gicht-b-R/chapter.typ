// Form Lab Pass R — Gicht · many small rasters (b-wave)
// lab: gicht-2026-08-b · pin: chapter-v-3-gicht.md
// Assets: domains/medical/assets/form-lab/gicht-2026-08-b/R/ (Semantic Read PASS)

#import "/packages/bookkit/lib.typ": *
#import "@preview/fontawesome:0.6.1": fa-icon, fa-version
#fa-version("6")

#set document(title: "Gicht — Form Lab Pass R", author: "form-lab/gicht-2026-08-b/R")
#set page(
  paper: "a4",
  ...
  header: context {
    ...
    grid(columns: (1fr, auto), [Gicht · Form Lab · Pass R (Raster aspects)], [viele kleine Kacheln · Typst-Text außerhalb])
  ...
  footer: context {
    ...
    line(length: 100%, stroke: 0.35pt + palette.border)
    v(3pt)
    grid(columns: (1fr, auto), [Form Lab · raster aspects · b-wave], counter(page).display("1"))
  },
)
...
#let aspect(path, w: 38mm, cap: none) = {
  block(
    width: w + 4mm,
    inset: 2pt,
    radius: 2pt,
...
// ── Recognition montage ───────────────────────────────────────
== Erkennung — Gestalt-Kacheln (heißes Mono)
...
#grid(
  ...
  align(center)[
    #aspect(
      "/domains/medical/assets/form-lab/gicht-2026-08-b/R/aspect-hot-mtp.jpg",
      w: 42mm,
      cap: [Kachel A · heißes MTP / Podagra-Gestalt],
    )
  ],
  align(center)[
    #aspect(
      "/domains/medical/assets/form-lab/gicht-2026-08-b/R/aspect-urate-needles.jpg",
      w: 42mm,
      cap: [Kachel B · Nadelkristalle (Polarisationsmikroskopie)],
    )
  ],
  align(center)[
    #aspect(
      "/domains/medical/assets/form-lab/gicht-2026-08-b/R/aspect-hot-knee.jpg",
      w: 42mm,
      cap: [Kachel C · heißes großes Gelenk (Knie)],
    )
  ],
)
...
#v(4pt)
#set text(size: 8.5pt)
*Claim-Trennung:* Bilder zeigen *Gestalt* (Rötung/Schwellung/Kristall-Morphologie) — sie *diagnostizieren* nicht Gicht vs. septisch. Kristallnachweis entwarnt septisch *nicht*. Kachel C: heißes großes Gelenk erinnert an CPPD/septisch am Knie — nicht „immer Gicht“.
...
2. *„Nur Gicht-Schub“* → Kristalle entwarnen septisch *nicht* (Kachel B ist Morphologie, kein „Alles klar“). \
3. *„Knie = Gicht“* → CPPD häufig; septisch Must-not-miss (Kachel C = Erinnerung, kein Beweis).
...
#grid(
  columns: (auto, 1fr),
  column-gutter: 8pt,
  aspect("/domains/medical/assets/form-lab/gicht-2026-08-b/R/aspect-hot-mtp.jpg", w: 32mm),
  text(size: 8.5pt)[
...
#table(
  ...
  [Erkennung], [Kacheln A–C Gestalt], [Gestalt ≠ Diagnose],
  [Handlung], [TIME lesbar], [HP-Grenze genannt],
...
#block(width: 100%, inset: 7pt, radius: 2pt, fill: rgb("#f8fafc"), stroke: 0.45pt + palette.border)[
  #set text(size: 8pt)
  #text(weight: "bold", fill: palette.primary)[Scan-Recap: ]
  Danger → Erkennung (Kacheln) → Mimic → Handlung → Grenze. Keine neuen Claims.
]
#v(4pt)
#align(center)[
  #text(size: 7.5pt, fill: palette.text-muted)[
    Form Lab Pass R (b-wave) · 3 Kacheln · Semantic Read PASS · not production Accept
  ]
]
