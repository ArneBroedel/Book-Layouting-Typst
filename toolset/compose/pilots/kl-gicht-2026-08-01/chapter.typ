...
// slug: kl-gicht-2026-08-01 · brief_class: exploration · clean slate
...
#import "/domains/medical/lib/typst/kl-wave6/gicht-2026-08-01.typ": *

#set document(
  title: "Gicht — kl-kurz exploration",
  author: "Book-Layouting-Typst · kl-gicht-2026-08-01",
)
...
#set page(
  ...
  header: context {
    ...
    grid(
      ...
      [Exploration `kl-gicht-2026-08-01`],
    )
...
// Slightly more air around H2 (Human: Gicht p1 a bit tight at headings)
#show heading.where(level: 2): it => {
  v(11pt, weak: true)
  block(below: 5pt, above: 2pt)[
...
// Layout revise 2026-08-01: ship *code* recognition panel (hybrid photo demoted —
// stock-photo break + duplicate danger band hurt scan-path; free PNG stays provenance)
#hot-joint-gicht(mode: "code")

#v(6pt)
#text(size: 9pt, weight: "bold", fill: palette.primary)[Vier Kanäle (Detail)]
#v(4pt)
#set text(size: 7.5pt)
// Layout revise: quiet definition grid (no hard table-wall relikt)
#grid(
  columns: (1fr, 1fr),
  column-gutter: 5pt,
  row-gutter: 5pt,
  block(width: 100%, inset: 6pt, radius: 2pt, fill: rgb("#f8fafc"), stroke: 0.4pt + palette.border)[
    ...
    #v(2pt)
    Hochakuter Mono-Gelenkschmerz binnen Stunden; oft MTP-I (Podagra); frühere Attacken; Alkohol, Diuretika, purinreiche Kost; Fieber? Hautverletzung?
  ],
  block(width: 100%, inset: 6pt, radius: 2pt, fill: rgb("#f8fafc"), stroke: 0.4pt + palette.border)[
