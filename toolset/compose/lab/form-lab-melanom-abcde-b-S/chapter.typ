...
#set document(title: "Melanom ABCDE — Form Lab Pass S", author: "form-lab/melanom-abcde-2026-08-b/S")
#set page(
  paper: "a4",
...
#grid(
  ...
  column-gutter: 8pt,
  image(A + "/aspect-lesion.jpg", width: 46mm),
...
// ── ABCDE cluster (quiet same-family chrome — not rainbow “Color” chips) ──
== ABCDE — Erkennungscluster (T/C quiet)

// Clinical “Color” (C) = melanin variety in the lesion image above, not UI hues.
#let abcde-chip(letter, body) = block(
  inset: 5pt,
  fill: rgb("#f0fdfa"),
  radius: 2pt,
  stroke: 0.4pt + palette.primary.lighten(35%),
)[
  #text(size: 8pt, weight: "bold", fill: palette.primary-dark, font: fonts.sans)[#letter]
  #v(2pt)
  #text(size: 7pt)[#body]
]
#grid(
  ...
  column-gutter: 3.5pt,
  abcde-chip([A], [Asymmetrie]),
  abcde-chip([B], [Border unregelmäßig]),
  abcde-chip([C], [≥2 Melanin-Töne (im Bild)]),
  abcde-chip([D], [Diameter oft ≥6 mm]),
  abcde-chip([E], [Evolution]),
)
