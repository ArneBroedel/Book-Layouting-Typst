...
5. **Graphic mode (visual units)** — set **track** (`free-vision-ambition` default for real visual units; `grammar-only` opt-in only). On ambition: **free vision** (Form Spec + pins; **no** layout-template clone) → **claim audit** → **harvest** (non-claim layout cues) → multi-round **realize**: hierarchy-faithful Typst recreate · true hybrid (labels **on** nodes) · AI refine · accept-asset. Prefer maintainable **code when ≈ equal after harvest**; **raster may be end product only with gates** (claim PASS, DPI/screen-only, MANIFEST, Accept). DRIFT ≠ discard composition. **Post-Accept free-vision proximity winners are authoritative** for Tech embed. Skill playbook: `domains/medical/skill/medical-graphics/playbook/`. Record `graphic_mode`, `form_spec_path`, `vision_path`, `harvest_path`, `realize_path`, track.
