...
2. **Table-default**  
   Mapping every structured intent to `styled-table` because it is easy. Compare → `comparison-table`; scores → `score-table`; protocols → `protocol-steps` (planned) or explicit gap — not a grey table.
...
5. **Bullet protocol**  
   Long clinical/technical SOPs as plain `-` lists with bold first words. That is a **gap** (`protocol-steps` planned), not a finished design.
