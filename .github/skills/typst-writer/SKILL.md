...
## 21 — Harvested Gotchas: Legal / Statute Text (Berufs- und Rechtskunde)

Distilled from the `legal-rechtskunde` devtrack (a `components/legal.typ` module + showcase chapter). All native to Typst 0.15 and token-bound via a `theme.legal` dict — packages exist (`rich-counters` for §→Abs→Satz numbering, `theorion`/`great-theorems` for numbered definition/case blocks, `glossarium`/`abbr` for abbreviation registers, `citrus` + `bibliography(style: "oscola.csl")` for legal CSL citation) but the didactic-box pattern is conflict-free and keeps colours/labels in tokens — prefer it unless auto-numbering across the whole document is required.

- **Inline norm citations must not break across lines.** Build a citation formatter that joins parts (`§ 203`, `Abs. 1`, `Satz 2`, `Nr. 3`, `lit. a`, gesetz-Kürzel LAST) and wrap the result in `box(text(…))` so `§ 203 Abs. 1 StGB` never splits mid-citation. Join with `sym.space.nobreak`.
- **Letter-suffixed sections (§ 630e, § 315a) are ONE identifier — pass `p: "630e"` (a string), not a separate field.** `"§ " + str(p)` then formats correctly; passing the letter via the gesetz/Abs field produces nonsense like "§ 630 e BGB".
- **Verbatim statute structure = hanging-indent grids, numbers passed explicitly.** Render Absatz/Nummer/Buchstabe with `grid(columns: (auto, 1fr), column-gutter: …)` (marker in col 1, text in col 2) and `pad(left: …)` to step nesting. Pass the original numbers literally — faithful quotation, not an auto-counter (which would renumber on edits).
- **Randnummern (marginal commentary numbers) need no margin package.** A `counter("randnummer")` + a reset helper + a two-column grid (narrow gutter column for the Rn, `1fr` for the text) gives the commentary look without `marginalia`'s layout fragility. `context counter.display()` inside the gutter cell.
- **Fixed multi-element diagrams must be `breakable: false`.** A Normenhierarchie pyramid (a `stack` of widening blocks) or any built-up figure will otherwise split across a page break, severing the shape — wrap the outer `block` with `breakable: false`.
- **Reuse the box spine, retheme the tokens.** `gesetzeskasten`, `rechtsprechung`, `legaldefinition`, `pruefungsschema`/`gutachten` are all the `didactic-box` pattern (`_label-chip` + left-accent `block`) driven by a `theme.legal` dict — one generic `_legal-box(kind, header, body)` plus thin wrappers, exactly as `didactics.typ` does it. Don't fork the box styling per legal element.
