...
- Mappt Brief → Realization (`mapped` / `custom` / `approx` / `gap` / `experiment`) + optional **`graphic_mode`**.  
- Optional **Spike**: kleines Experiment für neue/unsichere Wege → PDF/PNG (code, asset, ab-pair).  
...
- **Nicht:** Ideal ersetzen; Claims erfinden; Production-Accept allein; endlose Render-Loops.
...
[A] MEDIA — Media Brief (FREI, kein bookkit-Zwang)
        ...
        ▼
[B] TECH  — Feasibility note (mapped | custom | approx | gap | experiment)
        │    + graphic_mode on visual units
        │
        ├─► [B+] GRAPHICS — Decision Note / code|asset|hybrid|ab-test (optional)
        │
        ...
        └──────► [D] MEDIA — Accept / Revise / Reject
                      ...
                      │ accept
                      ▼
                 [C1] TECH Compose — .typ (full toolkit + accepted assets)
                      │
                      ▼
