...
- [x] Verify/refresh `domains/medical/OWNERSHIP.md` (B) — already exists  
- [x] Verify/refresh `domains/content-maturity/OWNERSHIP.md` (C)  
- [x] Add short `toolset/OWNERSHIP.md` or section in `toolset` README: product A, no medical SoT  
- [x] Add short `packages/OWNERSHIP.md` or bookkit README note: foundation vs didactics facet  
- [x] `research/README.md`: mark **R**, not default agent context for book production  
- [x] `domains/README.md`: one table A/B/C pointers  
...
- [x] Create chosen lib root + README (import examples, version note)
- [x] Move/copy **stable** modules; update spike imports to new path
- [x] Leave flaky `spike-*.typ` drivers in spikes/ or `experiments/`
- [x] Delete `_tmp/`, tracked PDFs already gone
- [x] Update `medical-graphics` skill “Modules” path line once libs move (can finish in phase4)
- [x] Compile at least one gold spike/recreate after move:
...
- [x] Keep **one** platform-relevant pilot: `pilots/kursbuch-welle-03-compose/` **or** document it as domain dogfood and add a **synthetic** A-only smoke under `toolset/examples/` (preferred long-term for A purity)
- [x] Mark `pilots/kursbuch-welle-03/` as legacy in README or move to `pilots/_archive/`
- [x] Ensure pilot README states: content SoT is external C

---

## 2.4 Research quarantine

- [x] Option 1: document “do not load research/ into agent default context” in AGENTS (phase4)
- [x] Option 2: git submodule later (phase3/R)
- [x] Minimum: `research/README.md` + `templates/README` warning banners

---

## 2.5 Asset / brief ongoing policy enforcement

- [x] Spot-check each gold unit `MANIFEST.md` lists only canonical files
- [x] Optional: archive non-gold **assets** for `kl-m-b-mimic`, `kl-mscc-spine` if not in gold set (or keep as optional gold support — note in INDEX)
- [x] Ensure new graphics scripts write under `assets/<slug>/` and do not auto-git-add

---

## Exit criteria

- [x] No A-foundation import of medical domain code
- [x] Stable graphic modules live in one documented lib path; spikes still compile
- [x] Pilot story clear (one primary + archive legacy)
- [x] B tree remains under `domains/medical/` and is subtree-split-ready (phase3 checklist)
